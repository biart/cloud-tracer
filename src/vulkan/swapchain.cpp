...
namespace ct
{
    namespace vulkan
    {
            ...
            device(device)
        {
            ...
            std::vector<std::uint32_t> unique_queue_family_indices; unique_queue_family_indices.reserve(3);
            if (device.SupportsCompute())
                PushBackUnique(unique_queue_family_indices, device.GetComputeQueueFamilyIndex());
            if (device.SupportsGraphics())
                PushBackUnique(unique_queue_family_indices, device.GetGraphicsQueueFamilyIndex());
            if (device.SupportsPresent())
                PushBackUnique(unique_queue_family_indices, device.GetPresentQueueFamilyIndex());
